...
public class TestKeywords
{
    ...
    public void testReservedWords()
    {
        for (int i = MIN_RESERVED_WORD + 1; i < MAX_RESERVED_WORD; i++) {
            Assertions.assertNull(parseStatement("select 1 as " + getTokenImageWithoutQuotes(i) + ";"), getTokenImageWithoutQuotes(i) + " should be a reserved word.");
        }
    }
    ...
    public void testNonReservedWords()
    {
        ...
        for (int i = MIN_NON_RESERVED_WORD + 1; i < MAX_NON_RESERVED_WORD - 1; i++) {
            Assertions.assertNotNull(parseStatement("select 1 as " + getTokenImageWithoutQuotes(i) + ";"), getTokenImageWithoutQuotes(i) + " should NOT be a reserved word.");
        }
    }
    ...
    public void testNoExtraneousKeywords()
    {
        for (int i = MAX_RESERVED_WORD + 1; i < tokenImage.length; i++) {
            ...
            Assertions.assertFalse(tokenImage[i].charAt(0) == '"' && Character.isLetter(tokenImage[i].charAt(1)), tokenImage[i] + " should be in one of the reserved word or non-reserved word group of tokens.");
        }
    }
}
